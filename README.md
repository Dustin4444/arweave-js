...
### ES Modules
The `web` build is published as native ES modules, so browsers can load it directly with `<script type="module">` — no bundler, and no bundle file.

It has a single external dependency, `bignumber.js`, which is left as a bare specifier. Browsers cannot resolve bare specifiers on their own, so add an [import map](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/script/type/importmap) pointing at its ES module build:
...
Nothing here requires a CDN. Both files can be served from your own origin — copy `web/` and `node_modules/bignumber.js/bignumber.mjs` out of the package and point the import map at your own paths:

```html
<script type="importmap">
{
  "imports": {
    "arweave": "/arweave/index.js",
